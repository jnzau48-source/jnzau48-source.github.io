...
// Initialize layers
for(let l=0;l<3;l++){
  let columns = Math.floor(canvas.width/fontSizes[l])*2;
  columnsLayers.push(columns);
  dropsLayers.push(Array(columns).fill(Math.floor(Math.random()*canvas.height/fontSizes[l])));
}
...
let current = 0; panels[current].classList.add('active');
function showPanel(index){
  panels.forEach((p,i)=>{p.classList.remove('active'); if(i===index)p.classList.add('active');});
}
window.addEventListener('wheel',e=>{
  current=e.deltaY>0?(current+1)%panels.length:(current-1+panels.length)%panels.length;
  showPanel(current);
  const logo=document.getElementById('far-wall-logo');
  if(logo){ logo.style.opacity=0.2+(current/panels.length)*0.3;
              logo.style.transform=`translateX(-50%) scale(${0.8+(current/panels.length)*0.2})`; }
});

// WhatsApp button
document.getElementById('whatsapp-btn').addEventListener('click',()=>{
  window.open('https://wa.me/254704813812','_blank');
});
